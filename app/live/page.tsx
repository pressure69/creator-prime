...
export default function LivePage() {
  ...
  const [aiEffectType, setAiEffectType] = useState('beauty');
...
if (!ctx) return;
    ...
    if (selectedFilter) {
      const effect = filterEffects[selectedFilter as keyof typeof filterEffects] || '';
      ctx.filter = `${effect} brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%)`;
      ctx.drawImage(videoElement, 0, 0);
    }
  };
